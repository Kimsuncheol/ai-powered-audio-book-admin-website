'use client';

import { useCallback, useEffect, useMemo, useState } from 'react';
import Box from '@mui/material/Box';
import CircularProgress from '@mui/material/CircularProgress';
import Typography from '@mui/material/Typography';
import ReviewList from '@/components/reviews/ReviewList';
import { useAuth } from '@/contexts/AuthContext';
import { listReviews } from '@/lib/reviews/reviewService';
import type { Actor, ReviewDocument } from '@/lib/types';

export default function ReviewsPage() {
  const { firebaseUser, role } = useAuth();
  const [reviews, setReviews] = useState<ReviewDocument[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [loadError, setLoadError] = useState<string | null>(null);

  const actor = useMemo<Actor | null>(
    () =>
      firebaseUser && role
        ? { uid: firebaseUser.uid, email: firebaseUser.email ?? '', role }
        : null,
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [firebaseUser?.uid, firebaseUser?.email, role]
  );

  const loadReviews = useCallback(async () => {
    if (!actor) return;
    setIsLoading(true);
    setLoadError(null);
    try {
      const data = await listReviews(
        { limit: 200, sortField: 'updatedAt', sortDirection: 'desc' },
        actor
      );
      setReviews(data);
    } catch (err) {
      setLoadError(err instanceof Error ? err.message : 'Failed to load reviews.');
    } finally {
      setIsLoading(false);
    }
  }, [actor]);

  useEffect(() => {
    if (!actor) return;
    loadReviews();
  }, [actor, loadReviews]);

  if (!actor) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', py: 8 }}>
        <CircularProgress />
      </Box>
    );
  }

  return (
    <Box>
      <Typography variant="h4" fontWeight={700} mb={3}>
        Reviews
      </Typography>
      <ReviewList reviews={reviews} isLoading={isLoading} loadError={loadError} />
    </Box>
  );
}
